...
SELECT o.id                                                                                      AS organization_id,
       ...
       CAST(
           COALESCE(COUNT(DISTINCT ti.id) FILTER (WHERE ti.status = 'Redeemed' AND rt.id IS NULL), 0) AS BIGINT)   AS redeemed_count,
       CAST(
           COALESCE(COUNT(DISTINCT ti.id) FILTER (WHERE ti.status in ('Purchased', 'Redeemed') AND rt.id IS NULL), 0) AS BIGINT)  AS purchased_count,
       CAST(
       ...
       CAST(COALESCE(
           COUNT(DISTINCT ti.id) FILTER (WHERE ti.hold_id IS NOT NULL AND h.hold_type = 'Comp' AND ti.status = 'Redeemed' AND rt.id IS NULL),
           0) AS BIGINT)                                                                AS comp_redeemed_count,
       CAST(COALESCE(
           COUNT(DISTINCT ti.id) FILTER (WHERE ti.hold_id IS NOT NULL AND h.hold_type = 'Comp' AND ti.status in ('Purchased', 'Redeemed') AND rt.id IS NULL),
