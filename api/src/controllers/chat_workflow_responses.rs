...
pub fn update(
    ...
    let updated_chat_workflow_response = chat_workflow_response.update(chat_workflow_response_parameters.into_inner(), connection)?;
...
pub fn destroy((conn, path, user): (Connection, Path<PathParameters>, User)) -> Result<HttpResponse, BigNeonError> {
    ...
    chat_workflow_response.destroy(Some(user.id()), &*conn)?;
